...
Publication and generation endpoints require `Authorization: Bearer <EVIDENCE_ENGINE_PUBLICATION_TOKEN>` and fail closed when the server secret is absent. The separate `/api/authoritative-results` facade is GET-only and verifies Cloudflare Access assertions server-side with pinned RS256, exact issuer/audience/hostname, time, subject, and email claims. Its deployment variables are `EVIDENCE_ENGINE_READER_HOST`, `CF_ACCESS_TEAM_DOMAIN`, and `CF_ACCESS_AUD`; all three must be configured together. There are no authoritative update or delete application routes. Manual deployment requires `EVIDENCE_ENGINE_D1_DATABASE_ID`; applying and verifying the checked-in D1 migrations is a separate default-off workflow input. The Worker publication secret is provisioned out of band and is never sent to browser code.

Company launch is authority-first: the client performs an as-of read followed by an integrity-verified exact-Result read. It does not acquire SEC evidence or run the evaluator when authority exists. Missing authority, failed reader authentication, and service failure are distinct UI states; acquisition begins only after the user explicitly chooses **Fresh / ad-hoc — not published**. Authoritative inputs remain read-only until that transition.
